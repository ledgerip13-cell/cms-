...
> **最后更新**：2026-07-16 (GMT+8)｜**对应提交**：待提交（X8 详情页按钮与选集规范修复）
...
- **2026-07-16 X8 详情页按钮与选集规范修复断点**：按参考站 `https://www.x8kb9k8.com/detail/145325` 的 `PlayBox / PlayListBox` 细节复核，修正 `web/src/x8/X8Home.vue` 详情页动作区和选集区：移除收藏/添加/分享按钮中的可见特殊符号，改为 60x60 SVG 图标按钮；主按钮调整为 160x60；详情统计栏、标签、导演/主演/别名/语言间距按参考站收紧；选集按钮统一为数字集数，64x40 紧凑按钮；播放器标题改为“线路名 播放器 / 排序”；footer 同步去掉特殊符号占位和版权符号，避免 X8 模板继续出现不规范符号。验证：`npm run build`（web）通过，`docker compose up -d --build web` 部署，`5150 /health` 正常，`5152` 新包 `assets/index-DIUORnK0.js`，浏览器硬刷 `#/x8/detail/921` 实测主按钮 160x60、动作按钮 60x60 SVG、集数按钮 64x40 数字、正文无旧符号，`git diff --check` 通过。
