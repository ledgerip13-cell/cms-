...
> **最后更新**：2026-07-16 (GMT+8)｜**对应提交**：待提交（X8 详情页细节二次对齐）
...
- **2026-07-16 X8 详情页按钮与选集规范修复断点**：按参考站 `https://www.x8kb9k8.com/detail/145325` 的 `PlayBox / PlayListBox` 细节复核，修正 `web/src/x8/X8Home.vue` 详情页动作区和选集区：移除收藏/添加/分享按钮中的可见特殊符号，改为 60x60 SVG 图标按钮；主按钮调整为 160x60 且强制 `20px/700/24px`，避免被全局 button 继承规则压回 16px；详情统计栏改为源站式“图标+数值/标题”上排与说明下排，热度显示为“热度 / 37人”；标签、导演/主演/别名/语言间距按参考站收紧；选集按钮统一为数字集数，64x40 紧凑按钮；播放器标题为“线路名 播放器”，右侧排序按钮支持升序/倒序切换；选集容器固定 `196px` 高，超出纵向滚动；footer 同步去掉特殊符号占位和版权符号，避免 X8 模板继续出现不规范符号。验证：`npm run build`（web）通过，`docker compose up -d --build web` 部署，`5150 /health` 正常，`5152` 新包 `assets/index-DanoMJ04.js`，浏览器硬刷 `#/x8/detail/921` 实测主按钮 160x60 且字体 `20px/700`、动作按钮 60x60 SVG、统计栏含评分/热度图标、选集固定 196px 滚动，排序从 `1/2/3/4/5` 切到 `149/148/147/146/145`，正文无旧符号，`git diff --check` 通过。
