...
- **2026-07-11 图标断点**：移动底栏“刷剧”图标已从手机外框+实心播放三角，改为和首页/剧场/我的一致的线性圆角短卡样式，避免形状重量突兀。
- **2026-07-11 全集态断点**：刷剧全集态简介、右侧交互区、打开声音/全屏观看提示整体上提，避免压进底部黑区、进度条和选集条。
- **2026-07-11 刷剧微调断点**：漫游态 `ms-roam-card` 高度调整为 46px，入口文字权重降为 500；漫游态标题/标签/简介位置为 `116px + safe-area`，全集态简介为 `70px + safe-area`，全集态右侧交互区为 `82px + safe-area`。
...
- **2026-07-11 交互/后台断点**：移动模板路由切换增加轻过渡，登录弹窗增加上浮弹出动画，刷剧筛选和选集抽屉改为底部弹出动画；后台影片线路诊断改为专用 `/api/admin/vods/:id/diagnose`，只检查线路解析，不再套用前台登录/VIP 权限；后台影片列表、详情和编辑弹窗已支持别名展示/维护，别名复用 `VodAlias.note` 保存展示名、`fingerprint` 保存去重指纹。
