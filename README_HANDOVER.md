...
> **最后更新**：2026-07-16 (GMT+8)｜**对应提交**：本次提交（X8 播放器弹窗与图标二次回修）
...
- **2026-07-16 X8 播放器弹窗与图标二次回修断点**：`web/src/x8/X8Home.vue` 按最新截图比例收回播放器内三类浮层尺寸：清晰度弹窗为约 `230x216`，三档两行卡片（`1080P/蓝光`、`720P/高清`、`480P/标清`）；设置弹窗为约 `302x188`，17px 文案、52x30 胶囊开关、毛玻璃背景；倍速二级面板为约 `268x338`，含 `0.5x/0.75x/1.0x/1.25x/1.5x/2.0x`。播放器图标重新按 lucide 语义区分：播放态显示 pause，暂停态显示 play；下一集使用 skip-forward；设置使用齿轮 settings，不再用闪电；HLS/video 原生全屏使用 fullscreen，全屏态切 minimize；系统/页面最大化使用 diagonal maximize，与 fullscreen 区分。控制图标实测 25px、设置 31px，按钮 hover `scale(1.12)`，进度条保持 2px 细轨道。已 `npm run build`、`docker compose up -d --build web`、`git diff --check` 通过；5150 `/health` 正常，5152 新包 `assets/index-DB8ph8t2.js / assets/index-CTbZVJT6.css`；浏览器实测 `#/x8/play/921` 切金牌后弹窗尺寸、图标路径和按钮 title 均符合预期。
