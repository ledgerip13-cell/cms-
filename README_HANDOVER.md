...
> **最后更新**：2026-07-16 (GMT+8)｜**对应提交**：待提交（X8 详情页多线路显示）
...
- **2026-07-16 X8 详情页按钮与选集规范修复断点**：按参考站 `https://www.x8kb9k8.com/detail/145325` 的 `InfoRight / PlayBox / PlayListBox` 细节复核，修正 `web/src/x8/X8Home.vue` 详情页信息区、动作区和选集区：左侧封面为 236x340，右侧 `.x8-detail-main` 锁定 `height/max-height:340px` 并 `justify-content:space-between`，去掉动作区额外顶距，保证右侧文案+按钮整体不超过封面图高度；移除收藏/添加/分享按钮中的可见特殊符号，改为 60x60 SVG 图标按钮；主按钮调整为 160x60 且强制 `20px/700/24px`；详情统计栏改为源站式“图标+数值/标题”上排与说明下排；选集区在标题下方新增多线路 tabs，渲染 `vod.lines` 全部线路，点击仅切换详情页选集，不跳播放；播放器标题为“当前线路 播放器”，右侧排序按钮支持升序/倒序切换；选集容器固定 `196px` 高，超出纵向滚动。验证：`npm run build`（web）通过，`docker compose up -d --build web` 部署，`5150 /health` 正常，`5152` 新包 `assets/index-BLKx5zrR.js`，浏览器硬刷 `#/x8/detail/921` 实测显示 15 条线路（无尽/魔都/1080网/如意/金牌等），点击“魔都”只切 active 线路且保持详情页 URL，选集仍固定 196px 滚动，正文无旧符号，`git diff --check` 通过。
