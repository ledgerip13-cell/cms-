...
- **2026-07-13 PWA 底栏 safe-area 二次回修断点**：上一版只用 `chromeReady` 延后显示，仍未根治真机 PWA 首载底栏偏上后归位。根因是 `.mtab` 高度与 padding 同时依赖 `env(safe-area-inset-bottom)`，iOS/PWA 首载 safe-area 重算会让底栏高度先变大再回落。现将 `MobileShell.vue` 的 `--mobile-tab-height` 固定为 `62px`，`.mtab` padding 固定为 `6px 14px`，底栏不再参与 safe-area 动态计算；漫游态底部条继续引用同一变量以保持贴合。已 `npm run build`、`docker compose up -d --build web` 通过；5152 390×844 视口实测 `/m`：`.mtab height=62px/bottom=0/paddingBottom=6px/top=782/bottom=844`。
